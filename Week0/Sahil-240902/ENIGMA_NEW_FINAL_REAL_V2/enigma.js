    ...
    canvas.addEventListener("mousedown", function (e) {
      ...
      for (let letter in keyMap) {
        ...
        if (dx * dx + dy * dy < KEY_RADIUS * KEY_RADIUS) {
          ...
          keySound.volume = 0.2;
          keySound.play().catch(e => console.error("Audio error:", e));
          
          // Apply plugboard mapping first
          let plugLetter = letter;
          if (plugboardPairs[letter]) {
            plugLetter = plugboardPairs[letter];
          }
          
          let index = alphabet.indexOf(plugLetter);
          
          // Advance the fast rotor (right rotor)
          advanceRotor(2);
    ...
    canvas.addEventListener("click", function (e) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (x > x1 && x < x2 && y > y1 && y < y2) {
          ...
          rotorSound.play().catch(e => console.error("Audio error:", e));
